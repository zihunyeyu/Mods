...
function RefreshBottomPanel()
	...
	-----
	if (m_SelectedGovernor ~= nil and m_SelectedGovernor:CanAssignToMajorCiv() and m_SelectedGovernorDef.GovernorType == 'GOVERNOR_THE_AMBASSADOR') then
		for i, pMajor in ipairs(PlayerManager.GetAliveMajors()) do
			...
			if ( playerDiplomacy:HasMet(pMajor:GetID()) ) then
				...
				local pCities = pMajor:GetCities();
				for i, pCity in pCities:Members() do
					...
					local playerGovernors = localPlayer:GetGovernors();
					if (pCity ~= nil) then
						local bCanAssign, results = playerGovernors:CanAssignGovernor(m_SelectedGovernorDef.Hash, pCity, true);
						bDisabled = not bCanAssign;
						if (results ~= nil) then
							local pFailureReasons : table = results[PlayerOperationResults.FAILURE_REASONS];
							if pFailureReasons ~= nil and table.count( pFailureReasons ) > 0 then
						...
						if m_SelectedGovernor:HasPromotion(GameInfo.GovernorPromotions['GOVERNOR_PROMOTION_AMBASSADOR_PUPPETEER'].Hash) then 
							AddCityInstance(pCity, bDisabled, sFailureText);
						end
					end
				end
			end
		end
	elseif (m_SelectedGovernor ~= nil and m_SelectedGovernor:CanAssignToMajorCiv()) then
		for i, pMajor in ipairs(PlayerManager.GetAliveMajors()) do
			local playerDiplomacy = localPlayer:GetDiplomacy();
			if ( playerDiplomacy:HasMet(pMajor:GetID()) ) then
				local pCapital = pMajor:GetCities():GetCapitalCity();
