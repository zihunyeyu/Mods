...
function RefreshGVNDPanel()
	Refresh();
end
ExposedMembers.ManYuDingZhi.RefreshGVNDPanel = RefreshGVNDPanel
...
function Refresh()
	...
	-- >>>>>>>>>>>>
	local bIsMerchant:boolean = IsCannotAssign(pGovernorDef) and (pGovernorDef.GovernorType == 'GOVERNOR_THE_MERCHANT');

	local governorPointsObtained = pPlayerGovernors:GetGovernorPoints();
	local governorPointsSpent = pPlayerGovernors:GetGovernorPointsSpent() + ExposedMembers.ManYuDingZhi.GetGovernorToEnvoysSpent(Game.GetLocalPlayer());
	...
	if not bIsSecretGovernor and not bIsMerchant then
		...
		if pGovernor then
			...
			local bCanAssign:boolean = bttPointsLeft and pPlayerGovernors:HasGovernor(pGovernorDef.Hash) and pGovernor:GetNeutralizedTurns() == 0;
			...
			local bCanAppoint:boolean = bttPointsLeft and pPlayerGovernors:CanAppoint();
	...
	if not bIsSecretGovernor or bIsMerchant then
		...
		if promotionSet.GovernorType == pGovernorDef.GovernorType then
			...
			if kPromotion.BaseAbility then
				...
				if kPromotion.Level ~= 0 then
					...
					local canEarnPromotion:boolean = pPlayerGovernors:CanEarnPromotion(pGovernorDef.Hash, kPromotion.Hash) and bttPointsLeft;

					if pGovernorDef.GovernorType =='GOVERNOR_THE_AMBASSADOR' then
						local bAmbassadorAppointed = playerGovernors:HasGovernor(pGovernorDef.Hash);
						if bAmbassadorAppointed then
							local bL:boolean = pGovernor:HasPromotion(DB.MakeHash('GOVERNOR_PROMOTION_AMBASSADOR_EMISSARY'));
							local bR:boolean = pGovernor:HasPromotion(DB.MakeHash('GOVERNOR_PROMOTION_AMBASSADOR_AFFLUENCE'));
							if bL and (kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_AMBASSADOR_AFFLUENCE' or kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_AMBASSADOR_FOREIGN_INVESTOR' or kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_AMBASSADOR_MINJIANJIAOLIU') then
								canEarnPromotion = false;
							elseif bR and (kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_AMBASSADOR_EMISSARY' or kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_LOCAL_INFORMANTS' or kPromotion.GovernorPromotionType == 'GOVERNOR_PROMOTION_AMBASSADOR_PUPPETEER') then 
								canEarnPromotion = false;
							end
						end
					end
...
function OnOpenDetails( governorIndex:number, isReadOnly : boolean )
	...
	m_isReadOnly = isReadOnly or (not bttPointsLeft);
