...
function RefreshGVNPanel()
	Refresh();
end
ExposedMembers.ManYuDingZhi.RefreshGVNPanel = RefreshGVNPanel
...
-- ===========================================================================
function AddMerchant(kGovernorDef, bCanAppoint, bIsUnlocked)
	local governorInstance = m_governorIM:GetInstance();

	-- Update name, title, and portrait
	governorInstance.GovernorName:SetText(Locale.ToUpper(kGovernorDef.Name));
	governorInstance.GovernorTitle:SetText(Locale.Lookup(kGovernorDef.Title));
	governorInstance.GovernorPortrait:SetTexture(kGovernorDef.PortraitImage);
	governorInstance.Labels:SetHide(true);

	local pPlayer = Players[Game.GetLocalPlayer()];
	local playerGovernors:object = pPlayer:GetGovernors();
	local pGovernor;
	local hasAllPromotions = true; 
	if bIsUnlocked then
		pGovernor = playerGovernors:GetGovernor(kGovernorDef.Hash);
	end

	-- Add all promotions this governor has
	for promotion in GameInfo.GovernorPromotionSets() do
		if promotion.GovernorType == kGovernorDef.GovernorType then
			local promotionDef = GameInfo.GovernorPromotions[promotion.GovernorPromotion];
			if bIsUnlocked then
				local hasPromotion:boolean = pGovernor:HasPromotion(DB.MakeHash(promotion.GovernorPromotion));
				if not hasPromotion then hasAllPromotions = false; end 

				AddPromotionInstance(governorInstance, kGovernorDef, promotionDef, hasPromotion);
			else 
				AddPromotionInstance(governorInstance, kGovernorDef, promotionDef, promotion.BaseAbility);
			end
		end
	end
	...
	if bIsUnlocked then
		...
		if bCanAppoint and not hasAllPromotions then
		...
		governorInstance.AppointButton:SetToolTipString(Locale.Lookup("LOC_GOVERNOR_MERCHANT_NEED_UNLOCK"));
	...
	governorInstance.GovernorDetailsButton:SetVoid1(kGovernorDef.Index);
...
function Refresh()
	...
	local bCanAppoint = playerGovernors:CanAppoint() and bttPointsLeft;
	local bHasGovernors, tGovernorList = playerGovernors:GetGovernorList();

	-- Mod
	Controls.GovernorPointsExchangeButton:SetDisabled(governorPointsObtained <= governorPointsSpent);
	...
	for _, pAppointedGovernor:table in ipairs(tGovernorList) do
		...
		local bCanPromoteGovernor:boolean = playerGovernors:CanPromoteGovernor(kGovernorDef.Hash) and bttPointsLeft;
		if (IsCannotAssign(kGovernorDef) and not (kGovernorDef.GovernorType == 'GOVERNOR_THE_MERCHANT')) then
	...
	for kGovernorDef in GameInfo.Governors() do
		if kGovernorDef.GovernorType == 'GOVERNOR_THE_MERCHANT' then
			AddMerchant(kGovernorDef, bCanAppoint, playerGovernors:HasGovernor(kGovernorDef.Hash));
		end
	end
