...
local m_kSupportedFormationClasses = {
...
function GetDataUnits()
	...
	local MaintenanceDiscountPerUnit = pTreasury:GetMaintDiscountPerUnit(); -- this will be used in 2 reports
	local pUnits                     = player:GetUnits();                -- 230425 moved
	local kUnitDataReport            = {};
	local group_name;
	local tUnitsDist                 = {}; -- temp table for calculating units' distance from cities

	--Timer2Start();
	for _, unit in pUnits:Members() do
		--print("unit", unit:GetID());
		local unitInfo = GameInfo.Units[unit:GetUnitType()];
		local formationClass = unitInfo.FormationClass; -- FORMATION_CLASS_CIVILIAN, FORMATION_CLASS_LAND_COMBAT, FORMATION_CLASS_NAVAL, FORMATION_CLASS_SUPPORT, FORMATION_CLASS_AIR
		-- categorize
		group_name = string.sub(formationClass, 17);
		if formationClass == "FORMATION_CLASS_CIVILIAN" then
			-- need to split into sub-classes
			if unit:GetGreatPerson():IsGreatPerson() then
				group_name = "GREAT_PERSON";
			elseif unitInfo.MakeTradeRoute then
				group_name = "TRADER";
			elseif unitInfo.Spy then
				group_name = "SPY";
			elseif unit:GetReligiousStrength() > 0 then
				group_name = "RELIGIOUS";
			end
		end
		...
		if kUnitDataReport[group_name] == nil then
			if group_name == "LAND_COMBAT" then
				kUnitDataReport[group_name] = {
					ID = 1,
					func = group_military,
					Header = "UnitsMilitaryHeaderInstance",
					Entry =
					"UnitsMilitaryEntryInstance"
				};
			elseif group_name == "NAVAL" then
				kUnitDataReport[group_name] = {
					ID = 2,
					func = group_military,
					Header = "UnitsMilitaryHeaderInstance",
			...
			elseif group_name == "SUPPORT" then
				kUnitDataReport[group_name] = {
					ID = 4,
					func = group_military,
					Header = "UnitsMilitaryHeaderInstance",
			...
			elseif group_name == "CIVILIAN" then
				kUnitDataReport[group_name] = {
					ID = 5,
					func = group_civilian,
					Header = "UnitsCivilianHeaderInstance",
					Entry =
					"UnitsCivilianEntryInstance"
				};
			elseif group_name == "RELIGIOUS" then
				kUnitDataReport[group_name] = {
					ID = 6,
					func = group_religious,
					Header = "UnitsReligiousHeaderInstance",
					Entry =
					"UnitsReligiousEntryInstance"
				};
			elseif group_name == "GREAT_PERSON" then
				kUnitDataReport[group_name] = {
					ID = 7,
					func = group_great,
					Header = "UnitsGreatPeopleHeaderInstance",
					Entry =
					"UnitsGreatPeopleEntryInstance"
				};
			elseif group_name == "SPY" then
				kUnitDataReport[group_name] = {
					ID = 8,
					func = group_spy,
					Header = "UnitsSpyHeaderInstance",
					Entry =
					"UnitsSpyEntryInstance"
				};
			elseif group_name == "TRADER" then
				kUnitDataReport[group_name] = {
					ID = 9,
					func = group_trader,
					Header = "UnitsTraderHeaderInstance",
					Entry =
					"UnitsTraderEntryInstance"
				};
			end
		...
		if bIsGatheringStorm then
			unit.ResMaint = "";
			local unitInfoXP2 = GameInfo.Units_XP2[unitInfo.UnitType];
	...
	for _, player in ipairs(PlayerManager.GetAlive()) do
		local bIsOurs = (player:GetID() == playerID);
		for _, city in player:GetCities():Members() do
			local iCityX, iCityY = city:GetX(), city:GetY();
			local sCityName = Locale.Lookup(city:GetName());
			local bIsCapital = city:IsCapital();
			for _, unit in ipairs(tUnitsDist) do
				local iDistance = Map.GetPlotDistance(unit:GetX(), unit:GetY(), iCityX, iCityY);
	...
	for _, instID in ipairs(GameEffects.GetModifiers()) do
		...
		local function RegisterModifierForUnit(iUnitID, sSubjectType, sSubjectName)
			...
			if data.Modifier == nil then
				print("WARNING! GetDataUnits/Modifiers: Ignoring non-existing modifier", data.ID, data.Definition.Id,
					sOwnerName, sSubjectName); return
			end
			data.UnitID = iUnitID;
			...
			if data.Modifier.EffectType == "EFFECT_GRANT_ABILITY" then
				local ability = data.Arguments.AbilityType;
				local unit = pUnits:FindID(iUnitID);
				if unit then
			...
			for _, subjectID in ipairs(tSubjects) do
				...
				if GameEffects.GetObjectsPlayerId(subjectID) == playerID then
					...
					if sSubjectType == "LOC_MODIFIER_OBJECT_UNIT" then
						...
						local iUnitOwnerID   = tonumber(string.match(sSubjectString, "Owner: (%d+)"));
		...
		local function IsRegistered(iUnitID)
		...
		if GameEffects.GetObjectsPlayerId(iOwnerID) == playerID and sOwnerType == "LOC_MODIFIER_OBJECT_UNIT" then
			...
			local iUnitOwnerID = tonumber(string.match(sOwnerString, "Owner: (%d+)"));
...
function GetDistrictIconForUnit(pUnit)
	...
	if GameInfo.DistrictReplaces[sDistrictType] then
		sDistrictType = GameInfo.DistrictReplaces[sDistrictType]
...
function sort_units(type, group, parent)
	...
	for _, unit in spairs(unit_group.units, function(t, a, b) return unit_sortFunction(parent.Descend, type, t, a, b) end) do
		...
		unitInstance.LookAtButton:RegisterCallback(Mouse.eLClick,
			function()
				Close(); UI.LookAtPlot(unit:GetX(), unit:GetY()); UI.SelectUnit(unit);
			end)
		unitInstance.LookAtButton:RegisterCallback(Mouse.eMouseEnter,
...
function common_unit_fields(unit, unitInstance)
	...
	local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas(
		"ICON_" .. UnitManager.GetTypeName(unit), 32)
	...
	unitInstance.UnitType:SetToolTipString(Locale.Lookup(GameInfo.Units[unit:GetUnitType()].Name) ..
		"[NEWLINE]" .. Locale.Lookup(GameInfo.Units[unit:GetUnitType()].Description));
	...
	local tUnitModifiers = m_kModifiersUnits[ unit:GetID() ];
	...
	local bIsMoving = true; -- Infixo

	if activityType == ActivityTypes.ACTIVITY_SLEEP then
		local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas("ICON_STATS_SLEEP", 22)
	...
	elseif activityType == ActivityTypes.ACTIVITY_HOLD then
		local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas("ICON_STATS_SKIP", 22)
		unitInstance.UnitStatus:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
	elseif activityType ~= ActivityTypes.ACTIVITY_AWAKE and unit:GetFortifyTurns() > 0 then
		local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas("ICON_DEFENSE", 22)
		unitInstance.UnitStatus:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
		bIsMoving = false;
	else
		-- just use a random icon for sorting purposes
		local textureOffsetX, textureOffsetY, textureSheet = IconManager:FindIconAtlas("ICON_STATS_SPREADCHARGES", 22)
		unitInstance.UnitStatus:SetTexture(textureOffsetX, textureOffsetY, textureSheet)
	...
	if (unit:GetFormationUnitCount() > 1) then
		unitInstance.UnitMove:SetText(tostring(unit:GetFormationMovesRemaining()) ..
	...
	elseif unitInstance.UnitMove then
		...
		unitInstance.UnitMove:SetText((bIsMoving and "[COLOR_Red]" or "") ..
			tostring(unit:GetMovesRemaining()) ..
			"/" .. tostring(unit:GetMaxMoves()) .. (bIsMoving and "[ENDCOLOR]" or ""))
	...
	local sCityName = (unit.NearCityIsCapital and "[ICON_Capital]" or "") .. unit.NearCityName;
	if unit.NearCityDistance == 0 then
		sCityName = "[COLOR:16,232,75,160]" .. sCityName .. "[ENDCOLOR]";
	elseif unit.NearCityIsOurs then
		sCityName = sCityName .. " " .. unit.NearCityDistance;
	else
		sCityName = "[COLOR_Red]" .. sCityName .. " " .. unit.NearCityDistance .. "[ENDCOLOR]";
	end
...
function group_military(unit, unitInstance, group, parent, type)
	...
	local eFormation = unit:GetMilitaryFormation();
	local iCombat, iRanged, iBombard = unit:GetCombat(), unit:GetRangedCombat(), unit:GetBombardCombat();

	-- name will be: name .. formation .. strength
	local sText = Locale.Lookup(unit:GetName());
	if eFormation == MilitaryFormationTypes.CORPS_FORMATION then
		sText = sText .. " [ICON_Corps]";
	elseif eFormation == MilitaryFormationTypes.ARMY_FORMATION then
		sText = sText .. " [ICON_Army]";
	end
	if iBombard > 0 then
		sText = sText .. " [ICON_Bombard]" .. tostring(iBombard);
	elseif iRanged > 0 then
		sText = sText .. " [ICON_Ranged]" .. tostring(iRanged);
	elseif iCombat > 0 then
		sText = sText .. " [ICON_Strength]" .. tostring(iCombat);
	end
	unitInstance.UnitName:SetText(sText);

	-- Level and Promotions
	local unitExp    = unit:GetExperience()
	local iUnitLevel = unitExp:GetLevel();
	if iUnitLevel < 2 then
		unitInstance.UnitLevel:SetText(tostring(iUnitLevel));
	elseif iUnitLevel == 2 then
		unitInstance.UnitLevel:SetText(tostring(iUnitLevel) .. " [ICON_Promotion]");
	else
		unitInstance.UnitLevel:SetText(tostring(iUnitLevel) .. " [ICON_Promotion]" .. string.rep("*", iUnitLevel - 2));
	end
	local tPromoTT = {};
	for _, promo in ipairs(unitExp:GetPromotions()) do
		table.insert(tPromoTT,
			Locale.Lookup(GameInfo.UnitPromotions[promo].Name) ..
			": " .. Locale.Lookup(GameInfo.UnitPromotions[promo].Description));
	end
	-- this section might grow!
	local tUnitModifiers = m_kModifiersUnits[unit:GetID()];
	--local isDebug = (unit:GetID() == 8781824); -- debug, will show only a specific unit
	local tMod = nil;
	local sText = "";
	if table.count(tUnitModifiers) > 0 then table.insert(tPromoTT, TOOLTIP_SEP); end
	local iPromoNum = 0;
	...
	for _, mod in ipairs(tUnitModifiers) do
		...
		local function AddExtraPromoText(sText)
			--if isDebug then print("AddExtraPromoText", mod.OwnerName, mod.Modifier.ModifierId, sText); end
			local sExtra = Locale.Lookup(mod.OwnerName) ..
				" (" .. RMA.GetObjectNameForModifier(mod.Modifier.ModifierId) .. ") " .. sText;
		...
		elseif tMod.EffectType == "EFFECT_ADJUST_UNIT_EXPERIENCE_MODIFIER" then
			AddExtraPromoText(string.format("%+d%% ", tonumber(tMod.Arguments.Amount)) ..
				Locale.Lookup("LOC_HUD_UNIT_PANEL_XP"));                                                      -- +x%
		...
		elseif tMod.EffectType == "EFFECT_ADJUST_UNIT_POST_COMBAT_HEAL" then
			AddExtraPromoText(Locale.Lookup("LOC_BRS_HEADER_HEALTH") ..
				string.format(" %+d", tonumber(tMod.Arguments.Amount))); -- +x HP
		elseif tMod.EffectType == "EFFECT_ADJUST_UNIT_BARBARIAN_COMBAT" then
			local iAdvStr = tonumber(tMod.Arguments.Amount);
			AddExtraPromoText(string.gsub(Locale.Lookup("LOC_COMBAT_PREVIEW_BONUS_VS_BARBARIANS", iAdvStr),
				"+" .. tostring(iAdvStr), "+" .. tostring(iAdvStr) .. " [ICON_Strength]"));                     -- +{1_Value} Advantage vs. Barbarians
	...
	unitInstance.UnitExp:SetText(tostring(unitExp:GetExperiencePoints()) ..
		"/" .. tostring(unitExp:GetExperienceForNextLevel()) .. ((bCanStart and tResults) and " [ICON_Promotion]" or ""));
	unitInstance.UnitExp:SetToolTipString((bCanStart and tResults) and
		Locale.Lookup("LOC_HUD_UNIT_ACTION_AUTOEXPLORE_PROMOTION_AVAILABLE") or "");

	-- Unit Health
	local iHealthPoints = unit:GetMaxDamage() - unit:GetDamage();
	local fHealthPercent = iHealthPoints / unit:GetMaxDamage();
	local sHealthColor = "";
	-- Common format is 0xBBGGRRAA (BB blue, GG green, RR red, AA alpha); stupid Firaxis - it's 0xAABBGGRR
	if fHealthPercent > 0.7 then
		sHealthColor = "[COLOR:16,232,75,160]"; -- COLORS.METER_HP_GOOD 0xFF4BE810
	elseif fHealthPercent > 0.4 then
		sHealthColor = "[COLOR:248,255,45,160]"; -- COLORS.METER_HP_OK   0xFF2DFFF8
	else
		sHealthColor = "[COLOR:245,1,1,160]";
	end -- COLORS.METER_HP_BAD  0xFF0101F5
	unitInstance.UnitHealth:SetText(sHealthColor ..
		tostring(iHealthPoints) .. "/" .. tostring(unit:GetMaxDamage()) .. "[ENDCOLOR]");
	...
	if (bCanStart) then
		...
		-- tooltip
		local upgradeToUnit = GameInfo.Units[tResults[UnitCommandResults.UNIT_TYPE]];
		local toolTipString = Locale.Lookup("LOC_UNITOPERATION_UPGRADE_INFO", Locale.Lookup(upgradeToUnit.Name),
			unit:GetUpgradeCost()); -- Upgrade to {1_Unit}: {2_Amount} [ICON_Gold]Gold
		-- Gathering Storm
...
function AddUpgradeResourceCost(pUnit, upgradeToUnitInfo)
	local toolTipString = "";
	-- requires
	...
	if (upgradeResource ~= nil and upgradeResource >= 0) then
		...
		toolTipString = toolTipString ..
			"[NEWLINE]" ..
			Locale.Lookup("LOC_UNITOPERATION_UPGRADE_RESOURCE_INFO", upgradeResourceCost, resourceIcon, resourceName); -- Requires: {1_Amount} {2_ResourceIcon} {3_ResourceName}
	end
	-- maintenance info
	local unitInfoXP2 = GameInfo.Units_XP2[upgradeToUnitInfo.UnitType];
	if unitInfoXP2 ~= nil then
		...
		if upgradeToUnitInfo.Maintenance > 0 then
			toolTipString = toolTipString ..
				" " ..
				Locale.Lookup("LOC_TOOLTIP_BASE_COST", upgradeToUnitInfo.Maintenance, "[ICON_Gold]",
					"LOC_YIELD_GOLD_NAME"); -- Base Cost: {1_Amount} {2_YieldIcon} {3_YieldName}
		end
		-- resources
		if unitInfoXP2.ResourceMaintenanceType ~= nil then
			local resourceName = Locale.Lookup(GameInfo.Resources[unitInfoXP2.ResourceMaintenanceType].Name);
			local resourceIcon = "[ICON_" .. GameInfo.Resources[unitInfoXP2.ResourceMaintenanceType].ResourceType .. "]";
			toolTipString = toolTipString ..
				" " ..
				Locale.Lookup("LOC_UNIT_PRODUCTION_FUEL_CONSUMPTION", unitInfoXP2.ResourceMaintenanceAmount, resourceIcon,
					resourceName); -- Consumes: {1_Amount} {2_Icon} {3_FuelName} per turn.
		end
	end
...
function ShowUnitPromotions(unit, unitInstance)
	-- Level and Promotions
	local tPromoTT = {};
	...
	if #tPromoTT == 0 then
		unitInstance.UnitLevel:SetText(sLevel);
	elseif #tPromoTT == 1 then
		unitInstance.UnitLevel:SetText(sLevel .. " [ICON_Promotion]");
	else
		unitInstance.UnitLevel:SetText(sLevel .. " [ICON_Promotion]" .. string.rep("*", #tPromoTT - 1));
	end
	unitInstance.UnitLevel:SetToolTipString(table.concat(tPromoTT, "[NEWLINE]"));
end
...
function group_trader(unit, unitInstance, group, parent, type)
	local owningPlayer = Players[unit:GetOwner()];
	local cities = owningPlayer:GetCities();
	--[[
	local yieldtype = {
	...
	for _, city in cities:Members() do
		...
		for i, route in ipairs(outgoingRoutes) do
			if unit:GetID() == route.TraderUnitID then
				...
				local destinationPlayer = Players[route.DestinationCityPlayer];
				local destinationCities = destinationPlayer:GetCities();
				local destinationCity = destinationCities:FindID(route.DestinationCityID);

				-- Set origin to destination name
				if originCity and destinationCity then
					unitInstance.UnitRoute:SetText(Locale.Lookup("LOC_HUD_UNIT_PANEL_TRADE_ROUTE_NAME",
						originCity:GetName(), destinationCity:GetName()))
					unit.route = Locale.Lookup("LOC_HUD_UNIT_PANEL_TRADE_ROUTE_NAME", originCity:GetName(),
				...
				for j, yieldInfo in pairs(route.OriginYields) do
					...
					if amount > 0 then
						yields = yields ..
							GameInfo.Yields[yieldInfo.YieldIndex].IconString .. toPlusMinusString(amount);
...
function ViewUnitsPage()
	...
	for iUnitGroup, kUnitGroup in spairs(m_kUnitDataReport, function(t, a, b) return t[b].ID > t[a].ID end) do
		...
		if pHeaderInstance.UnitTypeButton then
			pHeaderInstance.UnitTypeButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("type", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitNameButton then
			pHeaderInstance.UnitNameButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("name", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitStatusButton then
			pHeaderInstance.UnitStatusButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("status", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitLevelButton then
			pHeaderInstance.UnitLevelButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("level", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitExpButton then
			pHeaderInstance.UnitExpButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("exp", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitHealthButton then
			pHeaderInstance.UnitHealthButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("health", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitMoveButton then
			pHeaderInstance.UnitMoveButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("move", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitChargeButton then
			pHeaderInstance.UnitChargeButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("charge", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitYieldButton then
			pHeaderInstance.UnitYieldButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("yield", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitRouteButton then
			pHeaderInstance.UnitRouteButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("route", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitClassButton then
			pHeaderInstance.UnitClassButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("class", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitStrengthButton then
			pHeaderInstance.UnitStrengthButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("strength", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitSpreadButton then
			pHeaderInstance.UnitSpreadButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("spread", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitMissionButton then
			pHeaderInstance.UnitMissionButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("mission", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitTurnsButton then
			pHeaderInstance.UnitTurnsButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("turns", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitCityButton then
			pHeaderInstance.UnitCityButton:RegisterCallback(Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("city", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitMaintenanceButton then
			pHeaderInstance.UnitMaintenanceButton:RegisterCallback(
				Mouse.eLClick,
				function()
					instance.Descend = not instance.Descend; sort_units("maintenance", iUnitGroup, instance)
				end)
		end
		if pHeaderInstance.UnitAlbumsButton then
			if bIsGatheringStorm then
				pHeaderInstance.UnitAlbumsButton:RegisterCallback(Mouse.eLClick,
					function()
						instance.Descend = not instance.Descend; sort_units("albums", iUnitGroup, instance)
					end);
			else
