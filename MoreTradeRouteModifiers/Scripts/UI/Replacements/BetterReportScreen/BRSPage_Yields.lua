...
local DARKEN_CITY_INCOME_AREA_ADDITIONAL_Y = 6;
local SIZE_HEIGHT_BOTTOM_YIELDS            = 135;
local INDENT_STRING                        = "      ";
...
function GetDataYields()
    ...
    local isShowDetails                          = not Controls.HideCityBuildingsCheckbox:IsSelected(); -- read detailed data only when needed

    local kCityData                              = {};
    local kCityTotalData                         = {
        Income   = {},
        Expenses = {},
        Net      = {},
        Treasury = {}
    ...
    local pUnits                                 = player:GetUnits(); -- 230425 moved
    ...
    if isShowDetails then                                             -- modifiers are needed only when details are shown
        ...
        local sTrackedPlayer = PlayerConfigurations[playerID]:GetLeaderName(); -- LOC_LEADER_xxx_NAME
        --print("Tracking player", sTrackedPlayer); -- debug
        --for k,v in pairs(tTrackedEffects) do print(k,v); end -- debug
        local tTrackedOwners = {};
        ...
        for _, instID in ipairs(GameEffects.GetModifiers()) do
            local iOwnerID = GameEffects.GetModifierOwner(instID);
            local iPlayerID = GameEffects.GetObjectsPlayerId(iOwnerID);
            local sOwnerType = GameEffects.GetObjectType(iOwnerID);    -- LOC_MODIFIER_OBJECT_CITY, LOC_MODIFIER_OBJECT_PLAYER, LOC_MODIFIER_OBJECT_GOVERNOR
            local sOwnerName = GameEffects.GetObjectName(iOwnerID);    -- LOC_CITY_xxx_NAME, LOC_LEADER_xxx_NAME, etc.
            local tSubjects = GameEffects.GetModifierSubjects(instID); -- table of objectIDs or nil
            --print("checking", instID, sOwnerName, sOwnerType, iOwnerID, iPlayerID); -- debug

            local instdef = GameEffects.GetModifierDefinition(instID);
            local data = {
                ID = instID,
            ...
            local function RegisterModifierForCity(sSubjectType, sSubjectName)
                ...
                if data.Modifier == nil then
                    print("WARNING! GetDataYields/Modifiers: Ignoring non-existing modifier", data.ID, data.Definition
                        .Id, sOwnerName, sSubjectName); return
                end
            ...
            if sOwnerType == "LOC_MODIFIER_OBJECT_PLAYER" and sOwnerName == sTrackedPlayer and tSubjects then
                for _, subjectID in ipairs(tSubjects) do
                    local sSubjectType = GameEffects.GetObjectType(subjectID); -- LOC_MODIFIER_OBJECT_CITY, LOC_MODIFIER_OBJECT_PLAYER, LOC_MODIFIER_OBJECT_GOVERNOR
                    local sSubjectName = GameEffects.GetObjectName(subjectID); -- LOC_CITY_xxx_NAME, LOC_LEADER_xxx_NAME, etc.
                    if sSubjectType == "LOC_MODIFIER_OBJECT_CITY" and tTrackedOwners[sSubjectName] then
                        RegisterModifierForCity(sSubjectType, sSubjectName);
                    end
                end
            end
            ...
            if iPlayerID == playerID and sOwnerType == "LOC_MODIFIER_OBJECT_DISTRICT" and tSubjects then
                for _, subjectID in ipairs(tSubjects) do
                    ...
                    local sSubjectName = GameEffects.GetObjectName(subjectID);
                    if sSubjectType == "LOC_MODIFIER_OBJECT_DISTRICT" then
                        -- find a city
                        local sSubjectString = GameEffects.GetObjectString(subjectID);
                        local iCityID = tonumber(string.match(sSubjectString, "City: (%d+)"));
                        --print("city:", sSubjectString, "decode:", iCityID);
                        if iCityID ~= nil then
                            local pCity = player:GetCities():FindID(iCityID);
                            if pCity and tTrackedOwners[pCity:GetName()] then
                                RegisterModifierForCity(sSubjectType,
                                    pCity:GetName());
                            end
                        end
                    end
    ...
    for _, pCity in player:GetCities():Members() do
        ...
        local data                                                                             = GetCityData(pCity);
        data.WorkedTileYields, data.NumWorkedTiles, data.SpecialistYields, data.NumSpecialists = GetWorkedTileYieldData(
            pCity, pCulture); -- Add more data (not in CitySupport)

        -- Add to totals.
        kCityTotalData.Income[YieldTypes.CULTURE]                                              = kCityTotalData.Income
            [YieldTypes.CULTURE] + data.CulturePerTurn;
        kCityTotalData.Income[YieldTypes.FAITH]                                                = kCityTotalData.Income
            [YieldTypes.FAITH] + data.FaithPerTurn;
        kCityTotalData.Income[YieldTypes.FOOD]                                                 = kCityTotalData.Income
            [YieldTypes.FOOD] + data.FoodPerTurn;
        kCityTotalData.Income[YieldTypes.GOLD]                                                 = kCityTotalData.Income
            [YieldTypes.GOLD] + data.GoldPerTurn;
        kCityTotalData.Income[YieldTypes.PRODUCTION]                                           = kCityTotalData.Income
            [YieldTypes.PRODUCTION] + data.ProductionPerTurn;
        kCityTotalData.Income[YieldTypes.SCIENCE]                                              = kCityTotalData.Income
            [YieldTypes.SCIENCE] + data.SciencePerTurn;
        kCityTotalData.Income["TOURISM"]                                                       = kCityTotalData.Income
            ["TOURISM"] + data.WorkedTileYields["TOURISM"];

        kCityData[cityName]                                                                    = data;

        -- Add outgoing route data
        data.OutgoingRoutes                                                                    = pCity:GetTrade()
            :GetOutgoingRoutes();
        data.IncomingRoutes                                                                    = pCity:GetTrade()
            :GetIncomingRoutes();
        ...
        if eDominantReligion > 0 then -- WARNING! this rules out pantheons!
            for _, religionData in pairs(pCity:GetReligion():GetReligionsInCity()) do
                if religionData.Religion == eDominantReligion then
                    data.MajorityReligionFollowers = religionData
                        .Followers;
                end
            end
        end
        ...
        local pPlotCity = Map.GetPlot(pCity:GetX(), pCity:GetY());
        ...
        local iCurrentProductionHash = pCity:GetBuildQueue():GetCurrentProductionTypeHash();
        if iCurrentProductionHash ~= 0 then
            if GameInfo.Buildings[iCurrentProductionHash] ~= nil then
                data.CurrentProductionType = "BUILDING";
            elseif GameInfo.Districts[iCurrentProductionHash] ~= nil then
                data.CurrentProductionType = "DISTRICT";
            elseif GameInfo.Units[iCurrentProductionHash] ~= nil then
                data.CurrentProductionType = "UNIT";
            elseif GameInfo.Projects[iCurrentProductionHash] ~= nil then
                data.CurrentProductionType = "PROJECT";
            end
        end
        ...
        local tGrowthTT = {}; -- growth tooltip
        local function AddGrowthToolTip(sText, fValue, sSuffix)
            if fValue then
                table.insert(tGrowthTT,
                    Locale.Lookup(sText) .. ": " .. toPlusMinusString(fValue) .. (sSuffix and sSuffix or ""));
            else
                table.insert(tGrowthTT, Locale.Lookup(sText) .. ": " .. Locale.Lookup("LOC_HUD_CITY_NOT_APPLICABLE"));
            end
        end
        ...
        if data.TurnsUntilGrowth > -1 then
            ...
            AddGrowthToolTip("LOC_HUD_CITY_OCCUPATION_MULTIPLIER",
                (data.Occupied and data.OccupationMultiplier * 100) or nil, "%"); -- line 8a
            if bIsRiseFall or bIsGatheringStorm then
                if data.LoyaltyGrowthModifier ~= 0 then
                    AddGrowthToolTip(data.LoyaltyLevelName, data.LoyaltyGrowthModifier, "%"); -- line 8b
                else
                    table.insert(tGrowthTT,
                        Locale.Lookup(data.LoyaltyLevelName) ..
                        ": " .. Locale.Lookup("LOC_CULTURAL_IDENTITY_LOYALTY_NO_GROWTH_PENALTY"));
                end -- line 8b
            end
            AddGrowthToolTipSeparator();
            -- final
            AddGrowthToolTip("LOC_HUD_CITY_TOTAL_FOOD_SURPLUS", data.TotalFoodSurplus,
                (data.TotalFoodSurplus > 0 and "[ICON_FoodSurplus]") or "[ICON_FoodDeficit]"); -- line 9
            if data.Occupied then
                AddGrowthToolTip("LOC_HUD_CITY_GROWTH_OCCUPIED");                              -- line 10, occupied: no growth
            else
                table.insert(tGrowthTT,
                    Locale.Lookup("LOC_HUD_CITY_TURNS_UNTIL_CITIZEN_BORN", math.abs(data.TurnsUntilGrowth)));
            end -- line 10
        else
            ...
            table.insert(tGrowthTT,
                "[Color:StatBadCS]" .. string.upper(Locale.Lookup("LOC_HUD_CITY_STARVING")) .. "[ENDCOLOR]"); -- starving marker
            table.insert(tGrowthTT,
                Locale.Lookup("LOC_HUD_CITY_TURNS_UNTIL_CITIZEN_LOST", math.abs(data.TurnsUntilGrowth)));     -- line 10
        end
    ...
    -- NET = Income - Expense
    kCityTotalData.Net[YieldTypes.GOLD]         = kCityTotalData.Income[YieldTypes.GOLD] -
        kCityTotalData.Expenses[YieldTypes.GOLD];
    ...
    for i, pUnit in pUnits:Members() do
        ...
        local unitName              = Locale.Lookup(pUnitInfo.Name);
        ...
        local TotalMaintenanceAfterDiscount = math.max(GetUnitMaintenance(pUnit) - MaintenanceDiscountPerUnit, 0); -- cannot go below 0
        local unitTypeKey = pUnitInfo.UnitType .. unitMilitaryFormation;
        if kUnitData[unitTypeKey] == nil then
            kUnitData[unitTypeKey] = {
                Name = Locale.Lookup(pUnitInfo.Name),
                Formation = unitMilitaryFormation,
                Count = 1,
                Maintenance =
                    TotalMaintenanceAfterDiscount
            };
        ...
        if bIsGatheringStorm then
            ...
            local unitInfoXP2 = GameInfo.Units_XP2[pUnitInfo.UnitType];
            if unitInfoXP2 ~= nil and unitInfoXP2.ResourceMaintenanceType ~= nil then
                ...
                kUnitData[unitTypeKey].ResCount = kUnitData[unitTypeKey].ResCount + unitInfoXP2
        ...
        if otherID ~= playerID then
            ...
            if pDeals ~= nil then
                for i, pDeal in ipairs(pDeals) do
                    ...
                    local pOutgoingDeal = pDeal:FindItemsByType(DealItemTypes.GOLD, DealItemSubTypes.NONE, playerID);
                    if pOutgoingDeal ~= nil then
                        for i, pDealItem in ipairs(pOutgoingDeal) do
                            ...
                            if duration ~= 0 then
                                local gold = pDealItem:GetAmount();
                    ...
                    local pIncomingDeal = pDeal:FindItemsByType(DealItemTypes.GOLD, DealItemSubTypes.NONE, otherID);
...
function GetWorkedTileYieldData(pCity, pCulture)
    -- return data
    local kYields                                      = { YIELD_PRODUCTION = 0, YIELD_FOOD = 0, YIELD_GOLD = 0, YIELD_FAITH = 0, YIELD_SCIENCE = 0, YIELD_CULTURE = 0, TOURISM = 0 };
    local kSpecYields                                  = { YIELD_PRODUCTION = 0, YIELD_FOOD = 0, YIELD_GOLD = 0, YIELD_FAITH = 0, YIELD_SCIENCE = 0, YIELD_CULTURE = 0 };
    local iNumWorkedPlots                              = 0;
    local iNumSpecialists                              = 0;

    -- code partially taken from PlotInfo.lua
    local tParameters                                  = {};
    tParameters[CityCommandTypes.PARAM_MANAGE_CITIZEN] = UI.GetInterfaceModeParameter(CityCommandTypes
        .PARAM_MANAGE_CITIZEN);
    local tResults                                     = CityManager.GetCommandTargets(pCity, CityCommandTypes.MANAGE,
        tParameters);
    ...
    local tUnits = tResults[CityCommandResults.CITIZENS];
    --local tMaxUnits		 = tResults[CityCommandResults.MAX_CITIZENS]; -- not used
    --local tLockedUnits	 = tResults[CityCommandResults.LOCKED_CITIZENS]; -- not used
    ...
    for i, plotId in pairs(tPlots) do
        ...
        local index         = kPlot:GetIndex();
        local eDistrictType = kPlot:GetDistrictType();
        local numUnits      = tUnits[i];
        --local maxUnits = tMaxUnits[i];
...
-- ===========================================================================
local sortCities = { by = "CityName", descend = false }
...
local populationToScienceScale = GameInfo.GlobalParameters["SCIENCE_PERCENTAGE_YIELD_PER_POP"].Value / 100; -- Infixo added science per pop

function ViewYieldsPage()
    print("ViewYieldsPage", g_DirtyFlag.YIELDS, g_DirtyFlag.YIELDSDETAILS,
        Controls.HideCityBuildingsCheckbox:IsSelected());

    if g_DirtyFlag.YIELDS or g_DirtyFlag.YIELDSDETAILS then UpdateYieldsData(); end

    ResetTabForNewPageContent();

    local pPlayer = Players[Game.GetLocalPlayer()]; --BRS
    ...
    local goldCityTotal    = 0;
    local faithCityTotal   = 0;
    local scienceCityTotal = 0;
    local cultureCityTotal = 0;
    local tourismCityTotal = 0;
    ...
    local kBaseYields = GetEmptyYieldsTable();
    kBaseYields.TOURISM = 0;
    local function StoreInBaseYields(sYield, fValue) kBaseYields[sYield] = kBaseYields[sYield] + fValue; end

    -- ========== City Income ==========

    function CreatLineItemInstance(cityInstance, name, production, gold, food, science, culture, faith, bDontStore)
        local lineInstance = {};
    ...
    function SetFieldInLineItemInstance(lineItemInstance, yieldType, yieldValue)
        if yieldType == "YIELD_PRODUCTION" then
            lineItemInstance.Production:SetText(toPlusMinusNoneString(yieldValue));
        elseif yieldType == "YIELD_FOOD" then
            lineItemInstance.Food:SetText(toPlusMinusNoneString(yieldValue));
        elseif yieldType == "YIELD_GOLD" then
            lineItemInstance.Gold:SetText(toPlusMinusNoneString(yieldValue));
        elseif yieldType == "YIELD_FAITH" then
            lineItemInstance.Faith:SetText(toPlusMinusNoneString(yieldValue));
        elseif yieldType == "YIELD_SCIENCE" then
            lineItemInstance.Science:SetText(toPlusMinusNoneString(yieldValue));
        elseif yieldType == "YIELD_CULTURE" then
            lineItemInstance.Culture:SetText(toPlusMinusNoneString(yieldValue));
        end
    ...
    for cityName, kCityData in spairs(m_kCityData, function(t, a, b) return sortFunction(t, a, b) end) do --BRS sorting
        ...
        TruncateStringWithTooltip(pCityInstance.CityName, 230,
            (kCityData.IsCapital and "[ICON_Capital]" or "") .. Locale.Lookup(kCityData.CityName));
        pCityInstance.CityPopulation:SetText(kCityData.Population);

        --Great works
        local greatWorks = GetGreatWorksInCity(kCityData.City);

        -- Infixo reset base for amenities
        for yield, _ in pairs(kBaseYields) do kBaseYields[yield] = 0; end
        -- go to the city after clicking
        pCityInstance.GoToCityButton:RegisterCallback(Mouse.eLClick,
            function()
                Close(); UI.LookAtPlot(kCityData.City:GetX(), kCityData.City:GetY()); UI.SelectCity(kCityData.City);
            end);
        pCityInstance.GoToCityButton:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);

        -- Current Production
        local kCurrentProduction = kCityData.ProductionQueue[1]; -- this returns a table from GetCurrentProductionInfoOfCity() modified a bit in CitySupport.lua
        ...
        if kCurrentProduction ~= nil then
            ...
            local tooltip = kCurrentProduction.Name ..
                " [ICON_Turn]" ..
                tostring(kCurrentProduction.Turns) .. string.format(" (%d%%)", kCurrentProduction.PercentComplete * 100);
        ...
        local function SetYieldTextAndFocusFlag(pLabel, fValue, eYieldType)
            local sText = toPlusMinusString(fValue);
            local sToolTip = "";
        ...
        if not Controls.HideCityBuildingsCheckbox:IsSelected() then --BRS
            ...
            if kCityData.NumWorkedTiles > 0 then
                CreatLineItemInstance(pCityInstance,
                    ...
                    string.format("  [COLOR_White]%d[ENDCOLOR]", kCityData.NumWorkedTiles),
            ...
            if kCityData.NumSpecialists > 0 then
                CreatLineItemInstance(pCityInstance,
                    Locale.Lookup("LOC_BRS_SPECIALISTS") ..
                    string.format("  [COLOR_White]%d[ENDCOLOR]", kCityData.NumSpecialists),
            ...
            local tPopYields = GetEmptyYieldsTable(); -- will always show
            for _, mod in ipairs(kCityData.Modifiers) do
                if mod.Modifier.EffectType == "EFFECT_ADJUST_CITY_YIELD_PER_POPULATION" then
                    tPopYields[mod.Arguments.YieldType] = tPopYields[mod.Arguments.YieldType] +
                        kCityData.Population * tonumber(mod.Arguments.Amount);
                end
            end
            CreatLineItemInstance(pCityInstance,
                Locale.Lookup("LOC_HUD_CITY_POPULATION") ..
                string.format("  [COLOR_White]%d[ENDCOLOR]", kCityData.Population),
            ...
            for i, kDistrict in ipairs(kCityData.BuildingsAndDistricts) do
                ...
                function HasValidAdjacencyBonus(adjacencyTable)
                ...
                for i, kBuilding in ipairs(kDistrict.Buildings) do
                    ...
                    if greatWorks[kBuilding.Type] ~= nil then
                        ...
                        for _, kGreatWork in ipairs(greatWorks[kBuilding.Type]) do
                            local sIconString = GameInfo.GreatWorkObjectTypes[kGreatWork.GreatWorkObjectType].IconString;
                            local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                                INDENT_STRING .. INDENT_STRING .. sIconString .. Locale.Lookup(kGreatWork.Name), 0, 0, 0,
                                0, 0, 0);
            ...
            if kCityData.Wonders then
                for _, wonder in ipairs(kCityData.Wonders) do
                    if wonder.Yields[1] ~= nil or greatWorks[wonder.Type] ~= nil then
                        ...
                        local pLineItemInstance = CreatLineItemInstance(pCityInstance, wonder.Name, 0, 0, 0, 0, 0, 0);
                    ...
                    if greatWorks[wonder.Type] ~= nil then
                        ...
                        for _, kGreatWork in ipairs(greatWorks[wonder.Type]) do
                            local sIconString = GameInfo.GreatWorkObjectTypes[kGreatWork.GreatWorkObjectType].IconString;
                            local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                                INDENT_STRING .. sIconString .. Locale.Lookup(kGreatWork.Name), 0, 0, 0, 0, 0, 0);
                            for _, yield in ipairs(kGreatWork.YieldChanges) do
                                SetFieldInLineItemInstance(pLineItemInstance, yield.YieldType, yield.YieldChange);
                            end
                        end
            ...
            if kCityData.OutgoingRoutes then
                for i, route in ipairs(kCityData.OutgoingRoutes) do
                    if route ~= nil then
                        ...
                        local pOriginCity = CityManager.GetCity(route.OriginCityPlayer, route.OriginCityID)
                        local pDestCity = CityManager.GetCity(route.DestinationCityPlayer, route.DestinationCityID)
                        local cityTRMYields = GetCityTradeRouteModifierYieldsByTradeRouteID(pOriginCity, tradeRouteID)
                            [1]
                        local cityTRMDecimalYields = GetCityTradeRouteModifierYieldsByTradeRouteID(pOriginCity,
                            tradeRouteID, 'CityYieldsDecimal')[1]
                        --Assign yields to the line item
                        local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                            Locale.Lookup("LOC_HUD_REPORTS_TRADE_OUT_GOING", Locale.Lookup(pDestCity:GetName())), 0, 0, 0,
                            0,
                            0, 0);
                        for _, yieldInfo in ipairs(route.OriginYields) do
                            local yieldDetails = GameInfo.Yields[yieldInfo.YieldIndex];
                            if yieldDetails then
                                local amount = 0
                                ...
                                if cityTRMDecimalYields and cityTRMDecimalYields[yieldDetails.YieldType] then
                                    local modifierDecimalOutGoing = cityTRMDecimalYields[yieldDetails.YieldType]
                                        .BaseAmount
                                    ...
                                    amount = amount - modifierDecimalOutGoing
                                end

                                if not outGoingAmount[yieldDetails.YieldType] then
                                    outGoingAmount[yieldDetails.YieldType] = 0
                                end
                                outGoingAmount[yieldDetails.YieldType] = outGoingAmount[yieldDetails.YieldType] + amount
                                SetFieldInLineItemInstance(pLineItemInstance, yieldDetails.YieldType,
                                    amount + yieldInfo.Amount)
                            end
                        end
                    end
                end
            ...
            if kCityData.IncomingRoutes then
                for _, route in ipairs(kCityData.IncomingRoutes) do
                    if route ~= nil then
                        ...
                        local pOriginCity = CityManager.GetCity(route.OriginCityPlayer, route.OriginCityID)
                        local pDestCity = CityManager.GetCity(route.DestinationCityPlayer, route.DestinationCityID)
                        local cityTRMYields = GetCityTradeRouteModifierYieldsByTradeRouteID(pDestCity, tradeRouteID)[2]
                        local cityTRMDecimalYields = GetCityTradeRouteModifierYieldsByTradeRouteID(pOriginCity,
                            tradeRouteID, 'CityYieldsDecimal')[2]

                        --Assign yields to the line item
                        local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                            Locale.Lookup("LOC_HUD_REPORTS_TRADE_IN_COMING", Locale.Lookup(pOriginCity:GetName())), 0, 0,
                            0, 0,
                            0, 0);

                        for _, yieldInfo in ipairs(route.DestinationYields) do
                            local yieldDetails = GameInfo.Yields[yieldInfo.YieldIndex];
                            if yieldDetails then
                                local amount = 0
                                if cityTRMYields and cityTRMYields[yieldDetails.YieldType] then
                                ...
                                if not inComingAmount[yieldDetails.YieldType] then
                                    inComingAmount[yieldDetails.YieldType] = 0
                                end
                                inComingAmount[yieldDetails.YieldType] = inComingAmount[yieldDetails.YieldType] + amount
                                SetFieldInLineItemInstance(pLineItemInstance, yieldDetails.YieldType,
                                    amount + yieldInfo.Amount)
                            end
                        end
            ...
            local tFlatYields = GetEmptyYieldsTable();
            local bFlatYields = false;
            for _, mod in ipairs(kCityData.Modifiers) do
                if mod.Modifier.EffectType == "EFFECT_ADJUST_CITY_YIELD_CHANGE" then
                    tFlatYields[mod.Arguments.YieldType] = tFlatYields[mod.Arguments.YieldType] +
                        tonumber(mod.Arguments.Amount);
                    bFlatYields = true;
                end
            end
            for yieldType, amount in pairs(tFlatYields) do
                if outGoingAmount[yieldType] then
                    tFlatYields[yieldType] = tFlatYields[yieldType] - outGoingAmount[yieldType]
                end
                if inComingAmount[yieldType] then
                    tFlatYields[yieldType] = tFlatYields[yieldType] - inComingAmount[yieldType]
                end
            end

            local yieldSum = 0
            for key, value in pairs(tFlatYields) do
                yieldSum = yieldSum + value
            end
            bFlatYields = yieldSum ~= 0
            ...
            if bFlatYields then
                CreatLineItemInstance(
                    ...
                    tFlatYields.YIELD_PRODUCTION, tFlatYields.YIELD_GOLD, tFlatYields.YIELD_FOOD,
                    tFlatYields.YIELD_SCIENCE, tFlatYields.YIELD_CULTURE, tFlatYields.YIELD_FAITH
            ...
            if kCityData.NumSpecialtyDistricts > 0 then
                ...
                for _, mod in ipairs(kCityData.Modifiers) do
                    if mod.Modifier.EffectType == "EFFECT_ADJUST_CITY_YIELD_PER_DISTRICT" then
                        tFlatYields[mod.Arguments.YieldType] = tFlatYields[mod.Arguments.YieldType] +
                            tonumber(mod.Arguments.Amount) * kCityData.NumSpecialtyDistricts;
                        bFlatYields = true;
                    end
                end
                if bFlatYields then
            ...
            local tFollowersModifiers = GetEmptyYieldsTable(); -- not yields, but stores numbers anyway
            local bShowFollowers = false;
            for _, mod in ipairs(kCityData.Modifiers) do
                if mod.Modifier.EffectType == "EFFECT_ADJUST_FOLLOWER_YIELD_MODIFIER" then
                    tFollowersModifiers[mod.Arguments.YieldType] = tFollowersModifiers[mod.Arguments.YieldType] +
                        tonumber(mod.Arguments.Amount);
                    bShowFollowers = true;
                end
            end
            ...
            if bShowFollowers then
                CreatLineItemInstance(pCityInstance,
                    Locale.Lookup("LOC_UI_RELIGION_FOLLOWERS") ..
                    string.format("  [COLOR_White]%d[ENDCOLOR]", kCityData.MajorityReligionFollowers),
                    kBaseYields.YIELD_PRODUCTION * tFollowersModifiers.YIELD_PRODUCTION *
                    kCityData.MajorityReligionFollowers / 100.0,
                    ...
                    kBaseYields.YIELD_SCIENCE * tFollowersModifiers.YIELD_SCIENCE * kCityData.MajorityReligionFollowers /
                    100.0,
                    kBaseYields.YIELD_CULTURE * tFollowersModifiers.YIELD_CULTURE * kCityData.MajorityReligionFollowers /
                    100.0,
                    kBaseYields.YIELD_FAITH * tFollowersModifiers.YIELD_FAITH * kCityData.MajorityReligionFollowers /
                    100.0,
            ...
            local tPercYields = GetEmptyYieldsTable();
            local bPercYields = false;
            for _, mod in ipairs(kCityData.Modifiers) do
                if mod.Modifier.EffectType == "EFFECT_ADJUST_CITY_YIELD_MODIFIER" then
                    if tonumber(mod.Arguments.Amount) == nil then
                        ...
                        tPercYields[mod.Arguments.YieldType] = tPercYields[mod.Arguments.YieldType] +
                            tonumber(mod.Arguments.Amount);
                    end
                    bPercYields = true;
                end
            ...
            local fYieldPercent = kCityData.HappinessNonFoodYieldModifier / 100.0;
            local sModifierColor;
            if kCityData.HappinessNonFoodYieldModifier == 0 then
                sModifierColor = "COLOR_White";
            elseif kCityData.HappinessNonFoodYieldModifier > 0 then
                sModifierColor = "COLOR_Green";
            else
                sModifierColor = "COLOR_Red"; -- <0
            end
            local lineInstance = CreatLineItemInstance(pCityInstance,
                Locale.Lookup("LOC_HUD_REPORTS_HEADER_AMENITIES") ..
                string.format("  [" .. sModifierColor .. "]%+d%%[ENDCOLOR]", kCityData.HappinessNonFoodYieldModifier),
    ...
    local bHideFreeBuildings        = Controls.HideFreeBuildingsCheckbox:IsSelected();                                --BRS
    ...
    if GameCapabilities.HasCapability("CAPABILITY_REPORTS_UNIT_EXPENSES") then
        ...
        -- Units
        local iTotalUnitMaintenance = 0;
        local bHideFreeUnits = Controls.HideFreeUnitsCheckbox:IsSelected(); --BRS
        -- sort units by name field, which already contains a localized name, and by military formation
        for _, kUnitData in spairs(m_kUnitData, function(t, a, b)
            if t[a].Name == t[b].Name then
                return t[a].Formation <
                    t[b].Formation
            else
            ...
            if kUnitData.Maintenance ~= 0 or not bHideFreeUnits then
                ...
                if kUnitData.Formation == MilitaryFormationTypes.CORPS_FORMATION then
                    pUnitInstance.UnitName:SetText(kUnitData.Name .. " [ICON_Corps]");
                elseif kUnitData.Formation == MilitaryFormationTypes.ARMY_FORMATION then
                    pUnitInstance.UnitName:SetText(kUnitData.Name .. " [ICON_Army]");
                else
                    pUnitInstance.UnitName:SetText(kUnitData.Name);
                end
                ...
                if bIsGatheringStorm and kUnitData.ResCount > 0 then
                    pUnitInstance.UnitCount:SetText(kUnitData.Count ..
                        string.format(" /-%d%s", kUnitData.ResCount, kUnitData.ResIcon));
                end
    ...
    Controls.Scroll:SetSizeY(Controls.Main:GetSizeY() -
        (Controls.BottomYieldTotals:GetSizeY() + SIZE_HEIGHT_PADDING_BOTTOM_ADJUST));
...
function InitializeYields()
    ...
    Controls.HideCityBuildingsCheckbox:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end)
    ...
    Controls.HideFreeBuildingsCheckbox:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end)
    ...
    Controls.HideFreeUnitsCheckbox:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end)
