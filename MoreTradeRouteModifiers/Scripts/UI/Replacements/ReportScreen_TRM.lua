...
-- ===========================================================================
local m_debugNumResourcesStrategic         = 0; -- (0) number of extra strategics to show for screen testing.
local m_debugNumBonuses                    = 0; -- (0) number of extra bonuses to show for screen testing.
local m_debugNumResourcesLuxuries          = 0; -- (0) number of extra luxuries to show for screen testing.

-- ===========================================================================
--	CONSTANTS
-- ===========================================================================

local RELOAD_CACHE_ID                      = "ReportScreen"; -- Must be unique (usually the same as the file name)

local DARKEN_CITY_INCOME_AREA_ADDITIONAL_Y = 6;
local DATA_FIELD_SELECTION                 = "Selection";
local SIZE_HEIGHT_BOTTOM_YIELDS            = 135;
local SIZE_HEIGHT_PADDING_BOTTOM_ADJUST    = 85; -- (Total Y - (scroll area + THIS PADDING)) = bottom area
local INDENT_STRING                        = "        ";
local GOSSIP_ENTRY_CONSTANT                = 62;
local GOSSIP_HEADER_SIZE                   = 230;
local GOSSIP_FILTER_ALL                    = "ALL";
...
local m_strategicResourcesIM   = InstanceManager:new("ResourceAmountInstance", "Info", Controls.StrategicResources);
local m_bonusResourcesIM       = InstanceManager:new("ResourceAmountInstance", "Info", Controls.BonusResources);
local m_luxuryResourcesIM      = InstanceManager:new("ResourceAmountInstance", "Info", Controls.LuxuryResources);
local m_groupIM                = InstanceManager:new("GroupInstance", "Top", Controls.Stack); -- Collapsable

local m_kTabs                  = nil;
local m_CurrentReportTab       = 1;

local m_kResourceData          = nil;
local m_kCityData              = nil;
local m_kCityTotalData         = nil;
local m_kUnitData              = nil; -- TODO: Show units by promotion class
local m_kDealData              = nil;
local m_uiGroups               = nil; -- Track the groups on-screen for collapse all action.

local m_isCollapsing           = true;

--Gossip filtering
local m_kGossipInstances       = {};
local m_groupFilter            = GOSSIP_FILTER_ALL;
local m_leaderFilter           = -1; -- (-1) Indicates Unfilitered by this criteria (PlayerID)
local m_kGossipLog             = {};
local m_kGossipLogFiltered     = {};
local m_kGossipFiltersToShow   = {}; -- GroupType indexed table used to determine which gossip GroupType filters to show
local m_CountPerPage           = 0;
local m_CurrentGossipPage      = 0;
local m_MaxPages               = 0;
local m_uiPaginationInstance   = {};
local m_uiGossipFilterInstance = {};

local m_kGossipGroupTypes      = {};
...
function OnCollapseAllButton()
    ...
    Controls.CollapseAll:LocalizeAndSetText(m_isCollapsing and "LOC_HUD_REPORTS_EXPAND_ALL" or
        "LOC_HUD_REPORTS_COLLAPSE_ALL");
...
function GetData()
    ...
    for i, pCity in pCities:Members() do
        ...
        kCityTotalData.Income[YieldTypes.PRODUCTION] = kCityTotalData.Income[YieldTypes.PRODUCTION] +
            data.ProductionPerTurn;
        kCityTotalData.Income[YieldTypes.SCIENCE]    = kCityTotalData.Income[YieldTypes.SCIENCE] + data.SciencePerTurn;
        kCityTotalData.Income["TOURISM"]             = kCityTotalData.Income["TOURISM"] +
            data.WorkedTileYields["TOURISM"];
    ...
    for i, pUnit in pUnits:Members() do
        ...
        local TotalMaintenanceAfterDiscount = 0;
                    ...
                    if pOutgoingDeal ~= nil then
                        for i, pDealItem in ipairs(pOutgoingDeal) do
                            ...
                            if duration ~= 0 then
                                ...
                                local entryString = Locale.Lookup("LOC_HUD_REPORTS_ROW_DIPLOMATIC_DEALS") ..
                                    " (" ..
                                    Locale.Lookup(pPlayerConfig:GetPlayerName()) ..
                                    " " .. Locale.Lookup("LOC_REPORTS_NUMBER_OF_TURNS", remainingTurns) .. ")";
                                AddResourceData(kResources, resourceType, entryString, "LOC_HUD_REPORTS_TRADE_EXPORTED",
                                    -1 * amount);
                            end
                        end
                    end
                        ...
                        for i, pDealItem in ipairs(pIncomingDeal) do
                            ...
                            if duration ~= 0 then
                                ...
                                local entryString = Locale.Lookup("LOC_HUD_REPORTS_ROW_DIPLOMATIC_DEALS") ..
                                    ...
                                    Locale.Lookup(pPlayerConfig:GetPlayerName()) ..
                                    " " .. Locale.Lookup("LOC_REPORTS_NUMBER_OF_TURNS", remainingTurns) .. ")";
                                AddResourceData(kResources, resourceType, entryString, "LOC_HUD_REPORTS_TRADE_IMPORTED",
                                    amount);
                            end
                        end
                    end
            ...
            if suzerainID == playerID then
                for row in GameInfo.Resources() do
                    ...
                    if resourceAmount > 0 then
                        local pMinorPlayerConfig = PlayerConfigurations[pMinorPlayer:GetID()];
                        local entryString = Locale.Lookup("LOC_HUD_REPORTS_CITY_STATE") ..
                            " (" .. Locale.Lookup(pMinorPlayerConfig:GetPlayerName()) .. ")";
...
-- AddMiscResourceData(pResourceData, kResourceTable)


-- ===========================================================================
--	kResources	(in) the table to add resources to.
-- AddResourceData( kResources, eResourceType, EntryString, ControlString, InAmount)


-- ===========================================================================
--	Obtain the total resources for a given city.
-- ===========================================================================
-- GetCityResourceData( pCity )
...
--	Set + - in group row
-- ===========================================================================
function RealizeGroup(instance)
    local v = (instance["isCollapsed"] == false and instance.RowExpandCheck:GetSizeY() or 0);
    instance.RowExpandCheck:SetTextureOffsetVal(0, v);

    instance.ContentStack:CalculateSize();
    instance.CollapseScroll:CalculateSize();

    local groupHeight = instance.ContentStack:GetSizeY();
...
-- ===========================================================================
function OnToggleCollapseGroup(instance)
...
-- ===========================================================================
function OnAnimGroupCollapse(instance)
    -- Helper
    ...
    local groupHeight    = instance.ContentStack:GetSizeY();
    local collapseHeight = instance["CollapsePadding"] ~= nil and instance["CollapsePadding"] or 0;
    local startY         = instance["isCollapsed"] == true and groupHeight or collapseHeight;
    local endY           = instance["isCollapsed"] == false and groupHeight or collapseHeight;
    local progress       = instance.CollapseAnim:GetProgress();
    local sizeY          = lerp(startY, endY, progress);
...
-- ===========================================================================
function SetGroupCollapsePadding(instance, amount)
...
-- ===========================================================================
function NewCollapsibleGroupInstance(isCollapsed)
    if isCollapsed == nil then
        isCollapsed = false;
    end
    local instance = m_groupIM:GetInstance();
...
function ViewYieldsPage()
    ...
    function CreatLineItemInstance(cityInstance, name, production, gold, food, science, culture, faith)
        local lineInstance = {};
    ...
    for i, kCityData in ipairs(m_kCityData) do
        ...
        -- Current Production
        local kCurrentProduction = kCityData.ProductionQueue[1];
        pCityInstance.CurrentProduction:SetHide(kCurrentProduction == nil);
        if kCurrentProduction ~= nil then
            local tooltip = Locale.Lookup(kCurrentProduction.Name);
                ...
                if greatWorks[kBuilding.Type] ~= nil then
                    ...
                    for _, kGreatWork in ipairs(greatWorks[kBuilding.Type]) do
                        local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                            INDENT_STRING .. INDENT_STRING .. Locale.Lookup(kGreatWork.Name), 0, 0, 0, 0, 0, 0);
            ...
            for _, wonder in ipairs(kCityData.Wonders) do
                ...
                if greatWorks[wonder.Type] ~= nil then
                    ...
                    for _, kGreatWork in ipairs(greatWorks[wonder.Type]) do
                        local pLineItemInstance = CreatLineItemInstance(pCityInstance,
                            INDENT_STRING .. Locale.Lookup(kGreatWork.Name), 0, 0, 0, 0, 0, 0);
                        for _, yield in ipairs(kGreatWork.YieldChanges) do
                            if (yield.YieldType == "YIELD_FOOD") then
                                pLineItemInstance.Food:SetText(toPlusMinusNoneString(yield.YieldChange));
                            elseif (yield.YieldType == "YIELD_PRODUCTION") then
        ...
        if kCityData.OutgoingRoutes then
            for _, route in ipairs(kCityData.OutgoingRoutes) do
                -- =====================MODIFIER=====================
                if route ~= nil then
                    local tradeRouteID = table.concat({ route.OriginCityPlayer,
                        route.OriginCityID, route.DestinationCityPlayer,
                    ...
                    local cityTRMYields = GetCityTradeRouteModifierYieldsByTradeRouteID(pOriginCity, tradeRouteID)[1]
                    --Assign yields to the line item
                    local pLineItemInstance = CreatLineItemInstance(pCityInstance,
    ...
    for i, kCityData in ipairs(m_kCityData) do
        ...
        for _, kBuilding in ipairs(kCityData.Buildings) do
            if (kBuilding.Maintenance > 0 and kBuilding.isPillaged == false) then
                ...
                ContextPtr:BuildInstanceForControl("BuildingExpensesEntryInstance", pBuildingInstance,
        ...
        for _, kDistrict in ipairs(kCityData.BuildingsAndDistricts) do
            if (kDistrict.Maintenance > 0 and kDistrict.isPillaged == false and kDistrict.isBuilt == true) then
                local pDistrictInstance = {};
                ContextPtr:BuildInstanceForControl("BuildingExpensesEntryInstance", pDistrictInstance,
                    instance.ContentStack);
...
--	Allow for override.
-- ===========================================================================
-- function GetStrategicString( kSingleResourceData )
...
function ViewResourcesPage()
    ...
    for eResourceType, kSingleResourceData in pairs(m_kResourceData) do
        if next(kSingleResourceData.EntryList) then
            ...
            local kResourceEntries = kSingleResourceData.EntryList;
            for i, kEntry in ipairs(kResourceEntries) do
                local pEntryInstance = {};
                ...
                pEntryInstance.Amount:SetText((kEntry.Amount <= 0) and tostring(kEntry.Amount) or
                    "+" .. tostring(kEntry.Amount));
            end

            local pFooterInstance = {};
            ContextPtr:BuildInstanceForControl("ResourcesFooterInstance", pFooterInstance, instance.ContentStack);
            local valueString = tostring(kSingleResourceData.Total);
            ...
            local citiesProvidedTo = localPlayer:GetResources():GetResourceAllocationCities(GameInfo.Resources
                [kResource.ResourceType].Index);
            local numCitiesProvidingTo = table.count(citiesProvidedTo);
            if (numCitiesProvidingTo > 0) then
                pFooterInstance.AmenitiesContainer:SetHide(false);
                pFooterInstance.Amenities:SetText("[ICON_Amenities][ICON_GoingTo]" ..
                    Locale.Lookup("LOC_HUD_REPORTS_CITY_AMENITIES", numCitiesProvidingTo));
                local amenitiesTooltip = "";
                ...
                for i, city in ipairs(citiesProvidedTo) do
                    ...
                    amenitiesTooltip = amenitiesTooltip ..
                        city.AllocationAmount .. " [ICON_" .. kResource.ResourceType .. "] [Icon_GoingTo] " .. cityName;
                end
    ...
    for i, v in ipairs(kStrategics) do
        local resourceInstance = m_strategicResourcesIM:GetInstance();
    ...
    for i, v in ipairs(kBonuses) do
        local resourceInstance = m_bonusResourcesIM:GetInstance();
        resourceInstance.Info:SetText(v);
    end
    ...
    for i, v in ipairs(kLuxuries) do
        local resourceInstance = m_luxuryResourcesIM:GetInstance();
        resourceInstance.Info:SetText(v);
    end
    ...
    Controls.Scroll:SetSizeY(Controls.Main:GetSizeY() -
        (Controls.BottomResourceTotals:GetSizeY() + SIZE_HEIGHT_PADDING_BOTTOM_ADJUST));
end

-- ===========================================================================
function AddGossipForTarget(targetID)
    local bHasGossip = false;

    local kAppendTable = Game.GetGossipManager():GetRecentVisibleGossipStrings(0, Game.GetLocalPlayer(), targetID);
...
function ViewGossipPage()
    ...
    m_uiGossipFilterInstance.GroupFilter:RegisterSelectionCallback(function(i)
        local type = m_kGossipGroupTypes[i];
    ...
    for targetID, kPlayer in pairs(Players) do
        ...
        if targetID ~= playerID and kPlayer:IsMajor() and pLocalPlayerDiplomacy:HasMet(targetID) then
            --Append their gossip
            local bHasGossip = AddGossipForTarget(targetID);
            ...
            if bHasGossip then
                ...
                local leaderName = PlayerConfigurations[targetID]:GetLeaderTypeName();
                local iconName = "ICON_" .. leaderName;
                --Build and update
                local filterLeaderIcon = LeaderIcon:AttachInstance(uiFilter.LeaderIcon);
                ...
                if (not kPlayer:IsAlive()) then
                    ...
                    local leaderTooltip = uiFilter.LeaderIcon.Portrait:GetToolTipString();
    ...
    m_uiGossipFilterInstance.PlayerFilter:RegisterSelectionCallback(function(i)
        if i == -1 then
            ...
            local leaderName = PlayerConfigurations[i]:GetLeaderTypeName();
            local iconName = "ICON_" .. leaderName;
            --Build and update
            local filterLeaderIcon = LeaderIcon:AttachInstance(m_uiGossipFilterInstance.LeaderIcon);
            filterLeaderIcon:UpdateIcon(iconName, i, true);
            m_uiGossipFilterInstance.PlayerFilter:GetButton():SetText(Locale.Lookup(PlayerConfigurations[i]
                :GetLeaderName()));

            local leaderIsAlive = false;
            ...
            if (not leaderIsAlive) then
                local leaderTooltip = m_uiGossipFilterInstance.LeaderIcon.Portrait:GetToolTipString();
                leaderTooltip = leaderTooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_DEFEATED_SUBHEADER");
...
function UpdatePageInstance()
    ...
    m_uiPaginationInstance.FirstButton:RegisterCallback(Mouse.eMouseEnter,
    ...
    m_uiPaginationInstance.LastButton:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    ...
    if m_CurrentGossipPage - 2 > 0 then
        ...
        m_uiPaginationInstance.Button1:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    else
    ...
    if m_CurrentGossipPage - 1 > 0 then
        ...
        m_uiPaginationInstance.Button2:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    else
    ...
    if m_CurrentGossipPage > 0 then
        ...
        m_uiPaginationInstance.Button3:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    else
        --This is a fail case
    ...
    if m_CurrentGossipPage + 1 <= m_MaxPages then
        ...
        m_uiPaginationInstance.Button4:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    else
    ...
    if m_CurrentGossipPage + 2 <= m_MaxPages then
        ...
        m_uiPaginationInstance.Button5:RegisterCallback(Mouse.eMouseEnter,
            function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    else
    ...
    m_uiPaginationInstance.LeftArrow:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
    ...
    m_uiPaginationInstance.RightArrow:RegisterCallback(Mouse.eMouseEnter,
        function() UI.PlaySound("Main_Menu_Mouse_Over"); end);

    local kAliveLeaders = PlayerManager.GetAliveMajorIDs();
    local kGroupFiltersToShow = {};
    for i = 1, m_CountPerPage, 1 do
        local pageIndex = i + ((m_CurrentGossipPage - 1) * m_CountPerPage);
        if pageIndex <= table.count(m_kGossipLogFiltered) then
            local kGossipEntry = m_kGossipLogFiltered[pageIndex];
            local leaderName = PlayerConfigurations[kGossipEntry[4]]:GetLeaderTypeName();
            local iconName = "ICON_" .. leaderName;
            local pGossipInstance = m_kGossipInstances[i];

            pGossipInstance.Top:SetHide(false);

            local kGossipData = GameInfo.Gossips[kGossipEntry[3]];

            --Build and update
            local gossipLeaderIcon = LeaderIcon:AttachInstance(pGossipInstance.Leader);
            ...
            if (not leaderIsAlive) then
                ...
                local leaderTooltip = pGossipInstance.Leader.Portrait:GetToolTipString();
                leaderTooltip = leaderTooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_DEFEATED_SUBHEADER");
...
--	Filter Callbacks
-- ===========================================================================
function FilterGossip(kGossip)
    local kFilteredGossip = {};

    m_kGossipFiltersToShow = {};

    local gossipGroupType = m_groupFilter;
...
-- ===========================================================================
-- CreateCityStatusInstance( kParentInstance, kCityData )
