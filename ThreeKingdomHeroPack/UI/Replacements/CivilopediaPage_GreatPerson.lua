...
PageLayouts["GreatPerson"] = function(page)
	...
	if commandSQL and #commandSQL > 0 then
		...
		commandChages = commandSQL[1].ActionCharges
		commandName = Locale.Lookup('LOC_GREATPERSON_ACTION_NAME_DEFAULT')
		if command == 'UNITCOMMAND_CREATE_RESOURCE' then
			commandName = Locale.Lookup('LOC_UNITCOMMAND_CREATE_RESOURCE_DESCRIPTION')
		end

		commandDescription = GetCommandString(greatPersonType, command)
	end

	if commandName ~= '' and commandDescription ~= '' then
		table.insert(active_ability, commandDescription)
	end
							...
							if (args.Name == "ResourceType") then
								...
								if (resource) then
									table.insert(resources,
										{ { "ICON_" .. resource.ResourceType, resource.Name, resource.ResourceType },
											resource.Name });
								end
							end
						end
	...
	local has_active = (greatPerson.ActionCharges > 0 or commandChages > 0) and
		(#active_ability > 0 or greatPerson.ActionEffectTextOverride);
	-- =======================MODIFIER======================
	...
	if (has_active) then
		...
		local name = Locale.Lookup("LOC_UI_PEDIA_GREATPERSON_ACTION", active_name, greatPerson.ActionCharges);
		if greatPerson.ActionCharges == 0 and commandChages > 0 then
			name = Locale.Lookup("LOC_UI_PEDIA_GREATPERSON_ACTION", active_name, commandChages);
		end
		-- =======================MODIFIER======================
