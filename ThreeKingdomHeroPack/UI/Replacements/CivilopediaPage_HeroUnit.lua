-- ===========================================================================
--	Civilopedia - Hero Unit Page Layout
--	Heroes & Legends Mode addition based on CivilopediaPage_Unit
-- ===========================================================================
include("HeroesSupport");

PageLayouts["HeroUnit"] = function(page)
	...
	if (pGameHeroes ~= nil) then
		eHeroClass = pGameHeroes:GetUnitHeroClass(unit.Index);
				...
				for r in GameInfo.UnitReplaces() do
					if (r.ReplacesUnitType == u.UnitType) then
						...
						if (replaced_unit and matches_unique(replaced_unit)) then
							table.insert(upgrades_to,
								{ "ICON_" .. replaced_unit.UnitType, replaced_unit.Name, replaced_unit.UnitType });
						end
					end
				end
			end
	...
	for row in GameInfo.Unit_BuildingPrereqs() do
		if (row.Unit == unitType) then
			...
			if (building) then
				table.insert(requires_buildings, { "ICON_" .. building.BuildingType, building.Name, building
					.BuildingType });
			end
		end
	end
	...
	if (unit.PurchaseYield) then
		if (unit.PurchaseYield == "YIELD_GOLD") then
			purchase_cost = cost * GlobalParameters.GOLD_PURCHASE_MULTIPLIER *
				GlobalParameters.GOLD_EQUIVALENT_OTHER_YIELDS;
		else
	...
	AddRightColumnStatBox("LOC_UI_PEDIA_TRAITS", function(s)
		...
		if (unit.RangedCombat ~= 0) then
			s:AddIconNumberLabel({ "ICON_RANGED_STRENGTH", nil, "COMBAT_5" }, unit.RangedCombat,
				"LOC_UI_PEDIA_RANGED_STRENGTH");
		end
		...
		if (unit.ReligiousStrength ~= 0) then
			s:AddIconNumberLabel({ "ICON_RELIGION", nil, "FAITH_6" }, unit.ReligiousStrength,
				"LOC_UI_PEDIA_RELIGIOUS_STRENGTH");
		end

		if (unit.AntiAirCombat ~= 0) then
			s:AddIconNumberLabel({ "ICON_STATS_ANTIAIR", nil, "COMBAT_5" }, unit.AntiAirCombat,
				"LOC_UI_PEDIA_ANTIAIR_STRENGTH");
		end
		...
		local iLifespan = UnitManager.GetUnitTypeBaseLifespan(unit.Index);
		if (iLifespan > 0) then
			s:AddIconNumberLabel({ "ICON_LIFESPAN", nil, "HEROES" }, iLifespan, "LOC_HUD_UNIT_PANEL_LIFESPAN");
		end

		if (pGameHeroes ~= nil) then
			local iActionCharges = pGameHeroes:GetHeroClassBaseCharges(eHeroClass);
			if (iActionCharges > 0) then
				s:AddIconNumberLabel({ "ICON_STATS_SPREADCHARGES", nil, "HEROES" }, iActionCharges,
					"LOC_HUD_UNIT_PANEL_CHARGES");
			end
		end
		...
		if (unit.BuildCharges ~= 0) then
			s:AddIconNumberLabel({ "ICON_BUILD_CHARGES", nil, "IMPROVEMENTS" }, unit.BuildCharges,
				"LOC_UI_PEDIA_BUILD_CHARGES");
		end

		if (unit.ReligiousHealCharges ~= 0) then
			s:AddIconNumberLabel({ "ICON_RELIGION", nil, "FAITH_6" }, unit.ReligiousHealCharges,
				"LOC_UI_PEDIA_HEAL_CHARGES");
		end
	...
	if unitType == "UNIT_HERO_ARTHUR" then
		AddRightColumnStatBox("LOC_UI_PEDIA_CREATES", function(s)
			s:AddIconLabel({ "ICON_UNIT_QUESTING_KNIGHT", "LOC_UNIT_QUESTING_KNIGHT_NAME", "UNIT_QUESTING_KNIGHT" },
				"LOC_UNIT_QUESTING_KNIGHT_NAME");
			s:AddSeparator();
		end);
	end
	...
	if results then
		for _, row in ipairs(results) do
			if row.Name == 'UnitType' then
				createUnitType = row.Value
			end
		end
	end

	if createUnitType then
		AddRightColumnStatBox("LOC_UI_PEDIA_CREATES", function(s)
			s:AddIconLabel({ "ICON_" .. createUnitType, "LOC_" .. createUnitType .. "_NAME", createUnitType },
				"LOC_" .. createUnitType .. "_NAME");
			s:AddSeparator();
		end);
	end
		...
		if (unit.PrereqTech or unit.PrereqCivic or unit.PrereqDistrict or unit.StrategicResource or #requires_buildings > 0) then
			if (unit.PrereqDistrict ~= nil) then
				...
				if (district) then
					...
					s:AddIconLabel({ "ICON_" .. district.DistrictType, district.Name, district.DistrictType },
						district.Name);
				end
			end
			...
			if (unit.PrereqTech ~= nil) then
				...
				if (technology) then
					...
					s:AddIconLabel({ "ICON_" .. technology.TechnologyType, technology.Name, technology.TechnologyType },
						technology.Name);
				end
			end
			...
			if (unit.StrategicResource ~= nil) then
				...
				if (resource) then
					...
					s:AddIconLabel({ "ICON_" .. resource.ResourceType, resource.Name, resource.ResourceType },
						resource.Name);
				end
			end
			...
			for row in GameInfo.UnitOperations() do
				if (row.InterfaceMode == "INTERFACEMODE_SPY_CHOOSE_MISSION") then
					...
					if (row.TargetDistrict ~= nil) then
						s:AddLabel(Locale.Lookup("LOC_TYPE_TRAIT_TARGET_DISTRICT",
							GameInfo.Districts[row.TargetDistrict].Name));
					end
				end
			end
		end
	...
	-- Hero Abilities
	if (pGameHeroes ~= nil) then
		local eHeroClass = pGameHeroes:GetUnitHeroClass(unit.Index);
		local sAbilitiesString = FormatHeroClassAbilitiesAndCommands(eHeroClass);
		if (sAbilitiesString ~= nil and sAbilitiesString ~= "") then
			AddChapter("LOC_PEDIA_PAGE_CHAPTER_HEROABILITIES_TITLE", sAbilitiesString);
		end
	end
